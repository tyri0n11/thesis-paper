...
is the 96.3\% model agreement rate combined with a 12.1\% mean Jaccard
...
Online Service remained stable over the 54-day observation period without
memory growth, even at 30-second polling intervals across 20 concurrent routes.
...
over batch-based systems, where feature materialisation latency is governed by
the periodic aggregation schedule rather than event arrival
time~\cite{batch_latency_survey}. The Server-Sent Events delivery
...
Department of Transport would enable computation of true precision and recall.
...
Two suburban routes (zone4-to-zone2 and zone4-to-zone5) recorded a Z-score
anomaly rate of exactly 0.000 over the 54-day collection period, and
zone5-to-zone1 recorded a near-zero rate of 0.010. In low-variance
traffic conditions, the heavy-ratio baseline standard deviation may approach
zero, making the Z-score overly sensitive. The implementation applies a
minimum heavy-ratio stddev floor of \texttt{0.01} (with a \texttt{mean * 0.2}
fallback when only sparse history is available), but this is still a heuristic
that may generate spurious signals immediately after the baseline is first
computed.

\textbf{Hardware constraints.}
The platform is deployed on a single-node homelab (Intel Core i5-8500,
6~cores at 3.0~GHz; 24~GB DDR4 RAM; NVIDIA GeForce GTX~1060 6~GB VRAM).
Horizontal scaling of the Serving API and Batch Service to multiple nodes
was not evaluated. The Redpanda broker operates as a single-partition
configuration; a multi-partition, multi-replica setup would be required
for fault-tolerant production deployments at city scale.
...
Table~\ref{tab:comparison} reproduces the architectural comparison from
Table~\ref{tab:1} (Chapter~2) and adds the measured ingestion latency.
Column labels and baseline capability assessments are consistent with the
Related Work chapter.

\begin{table}[htbp]
\centering
\caption{Architectural comparison of UrbanPulse against reference pipeline patterns
         (\protect\cmark~yes, \protect\xmark~no).}
...
Schema Evolution \& Time-Travel~\cite{iceberg-table-format}               & \cmark & \xmark & \xmark & \xmark \\
Versioned Catalog (Git-like semantics)~\cite{nessie-catalog}              & \cmark & \xmark & \xmark & \xmark \\
Ingestion latency (p95)                                                   & $<$20 s & schedule-bounded & seconds--minutes & seconds--minutes \\
\hline
\end{tabular}%
}
\end{table}

\noindent UrbanPulse is the only pattern that combines both processing layers
with native schema evolution and versioned cataloguing, at a measured p95
latency of under 20 seconds. Beyond architectural properties, UrbanPulse
further contributes a dual-signal anomaly detector (Z-score combined with
Isolation Forest) and an LLM-based explainability layer --- capabilities
orthogonal to the choice of pipeline architecture. Its primary limitation
relative to batch-oriented systems is the absence of ground-truth evaluation,
a constraint shared by most deployed streaming anomaly detectors that operate
on unlabelled real-world data.
...
\begin{itemize}
  ...
  \item \textbf{Online learning for IsolationForest}: Replace the 6-hour
        ...
        detector~\cite{tan2011hst} to reduce concept drift exposure.
