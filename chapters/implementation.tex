...
model to produce operator-ready, natural-language explanations such as:

\begin{quote}
\textit{``The Southern Coastal $\to$ Western Periurban route is showing
abnormal heavy congestion (10.9\%) at 15:00, significantly above the
historical average of 4.2\% for Friday afternoons. Current weather data
indicates 12.4~mm rainfall, which typically correlates with a 1.8$\times$
surge in heavy vehicle ratios on Quốc lộ 50. This is consistent with
three similar events indexed in the last 7 days.''}
\end{quote}
...
(thumbs-up / thumbs-down) further closes the loop by collecting
operator judgements for future retrieval ranking improvement.
...
Clicking the \textbf{Phân tích} button triggers a full-heatmap RAG query that
aggregates all active anomaly signals and returns a structured four-part
analysis (Figure~\ref{fig:ai-heatmap-analysis}): \textit{(1)} multi-signal
