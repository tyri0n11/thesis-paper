...
The \textbf{Streaming Service} subscribes to \texttt{traffic-route-bronze}
(consumer group \texttt{streaming-group}) and writes each message as a
Parquet file to MinIO under the time-partitioned path:

\begin{center}
\texttt{s3://urban-pulse/bronze/traffic-route-bronze/year=\{Y\}/month=\{M\}/day=\{D\}/hour=\{H\}/\{uuid\}.parquet}
\end{center}
...
The \texttt{baseline\_learning.run()} function queries the entire Gold table
and computes, per \texttt{(route\_id, day\_of\_week, hour\_of\_day)} triple,
...
Each per-route model is trained with
\texttt{contamination=0.05} (5\% of training hours assumed anomalous),
\texttt{n\_estimators=100}, and \texttt{random\_state=42}~\cite{liu2008isolation}.
The model is registered in MLflow under the name \texttt{iforest-\{route\_id\}}
with alias \texttt{champion}, enabling the Serving API to load the latest
production-ready model via a single \texttt{mlflow.pyfunc.load\_model} call
with a 1-hour in-process cache (\texttt{MODEL\_TTL = 3600~s}).
