...
\begin{enumerate}
  ...
  \item \textbf{Stream-batch consistency:} Because the batch pipeline
        commits to Iceberg atomically (all rows of a micro-batch in a
        single snapshot), any downstream reader always observes a
        \textit{complete}, consistent view of the Gold table --- never a
        partial write mid-aggregation. This eliminates the
        \textit{phantom read} problem that arises when streaming systems
        read from mutable database tables. The concrete consumer of this
        guarantee is the baseline-loading mechanism described in
        Section~\ref{sec:impl-batch}.
\end{enumerate}
