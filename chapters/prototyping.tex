...
(8~GB VRAM). \textbf{Qwen2.5:3b}~\cite{qwen25_report}, served via
\textbf{Ollama}, achieves an average time-to-first-token of under one second
while producing coherent 2--3 sentence traffic explanations in both English
and Vietnamese. The \textit{RAG-over-fine-tuning} approach is justified by the
need for always-current context: retraining on new anomaly events would be
prohibitively expensive on the available hardware.
...
\begin{itemize}
  \item \textbf{\texttt{route\_id}}: unique string identifier of the inter-zone
        route (e.g., \texttt{zone1\_urban\_core\_to\_zone2\_eastern\_innovation}).
...
\begin{enumerate}
  ...
  \item \textbf{Gold} (\texttt{gold.traffic\_hourly}): 11-column hourly
        aggregation. Fields include \texttt{avg\_duration\_minutes},
        \texttt{p95\_duration\_minutes}, \texttt{avg\_heavy\_ratio},
        \texttt{avg\_moderate\_ratio}, and \texttt{max\_severe\_segments}.
...
\begin{itemize}
  \item \texttt{docker-compose.base.yaml} --- shared service definitions
        (Redpanda, MinIO, Nessie, PostgreSQL, ChromaDB, MLflow, Prefect, Ollama).
  \item \texttt{docker-compose.dev.yaml} --- development overlay: exposes ports
        locally, mounts source code volumes, uses \texttt{host.docker.internal}
        for the Ollama endpoint.
  \item \texttt{docker-compose.prod.yaml} --- production overlay: adds
