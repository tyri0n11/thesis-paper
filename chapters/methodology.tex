  ...
  Hourly weather data for Ho Chi Minh City (latitude $10.7757^\circ$N,
  longitude $106.7009^\circ$E, centred on Bến Thành district) is retrieved
  from the \textbf{Open-Meteo API} \cite{openmeteo_api}, a free,
  open-source meteorological service requiring no authentication.
  Retrieved variables include: temperature ($^\circ$C), precipitation
  ($\text{mm}$), rainfall ($\text{mm}$), wind speed ($\text{km/h}$),
  wind direction ($^\circ$), cloud cover ($\%$), and WMO weather code.                                                                                         
                                                                                                                                                          
  Weather data is consumed in two operationally distinct modes:                                                                                           
                                                                                                                                                          
  \begin{enumerate}                                                                                                                                       
      \item \textbf{Batch indexing mode:} The \texttt{hourly-gold}
      Prefect flow fetches the trailing 7 days of hourly records                                                                                          
      ($\approx 192$ documents) and indexes them into the                                                                                                 
      \texttt{external\_\allowbreak context} ChromaDB collection for RAG retrieval                                                                                    
      (Section~\ref{sec:rag}).                                                                                                                            
                                                                                                                                                          
      \item \textbf{Live fetch mode:} On each \texttt{/chat} API request,                                                                                 
      the serving layer performs a direct asynchronous HTTP fetch of current                                                                              
      conditions, cached in-process for 15 minutes, and injects the result                                                                                
  ...
  where $\theta_r$ is a \textbf{dynamic, per-route threshold} derived                                                                                     
  from the 99th percentile of the baseline distribution (default
  $\theta_r = 2.0$). The threshold is \textbf{one-sided}: only positive                                                                                   
  deviations are flagged, as a lower-than-baseline heavy ratio indicates                                                                                  
  free-flowing traffic, which does not constitute an anomalous condition.                                                                                 
  Algorithm~\ref{alg:zscore} formalises the full per-message classification                                                                               
  procedure.                                                                                                                                              
  ...
  produces a continuous anomaly score $s(\mathbf{x}) \in [-1, 1]$
  ...
  \begin{algorithm}[H]
      ...
      \begin{algorithmic}[1]
          ...
          (\mathbf{X}_{\text{feat}})$
          \Comment{Returns $s_r \in [-1,\,1]$; lower = more anomalous}
  ...
  dedicated \texttt{route\_iforest\_\allowbreak scores} PostgreSQL table via
  the SSE \texttt{/events/traffic} endpoint. This explicit                                                                                                
  ...
  \texttt{external\_context} & 7-day HCMC hourly weather                                                                                                  
                                $\approx$192 docs  & Open-Meteo   & 1 hour \\                                                                             
  \hline                                                                                                                                                  
  \end{tabular}
  }%
  \end{table}                                                                                                                                             
