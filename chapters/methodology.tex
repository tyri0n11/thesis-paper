  ...
  Ho Chi Minh City presents a particularly demanding environment: a road                                                                                  
  network of over 4,000 kilometres, a registered vehicle population exceeding                                                                             
  8 million units, and peak-hour congestion patterns that shift substantially                                                                             
  across zones, time of day, and meteorological conditions \cite{hcmc_transport_report}.                                                                  
  ...
  Figure~\ref{fig:zscore_timeseries} illustrates the Z-score detector
  operating across all 20 monitored routes over a 24-hour window.
  ...
  as sine-cosine pairs \cite{cyclical_encoding} projects circular
  ...
  Figure~\ref{fig:mlflow_experiments} shows the MLflow experiment
  view after a full retraining cycle, with one registered run per
  directed route. Each run records the hyperparameters, training
  data time range, and evaluation metrics, and is promoted to the
  \texttt{@champion} alias upon completion.
  ...
  Figure~\ref{fig:mlflow_metrics} shows the model metrics logged
  for a representative route
  (\textit{Southern Coastal~$\to$~Western Peri-urban}).
  The \texttt{iforest\_anomaly\_rate} of $0.05$ confirms that the
  contamination prior is honoured exactly by the decision threshold.
  The \texttt{model\_agreement} of $0.98$ indicates that the Z-score
  and IForest signals agree on 98\% of observations, with divergence
  concentrated in the edge cases each signal is designed to capture.
  ...
  retrieval budgets and $\text{hr}, \text{dow}$ are the current                                                                                           
  ...
  IForest inference latency & \texttt{/predict/anomalies}                                                                                                 
                              request $\to$ response                                                                                                      
                            & $< 2\,\text{s}$                                                                                                             
                            & Serving API logs \\                                                                                                         
