  ...
  anomalous. The fallback of $2.0$ applies when the baseline row exists but
  \texttt{zscore\_p99} is unavailable (for example, sparse history). If no
  baseline entry exists for the current route/time slot, the Online Service
  returns \texttt{None} and does not emit a Z-score anomaly. The threshold is
  \textbf{one-sided}:
  ...
  pronounced spikes that exceed their dynamic route thresholds during the
  06:00 and 16:00--17:00 peak-hour windows --- periods consistent with
  ...
  \begin{figure}[htbp]
      ...
      corresponds to the fallback default applied when the baseline row
      lacks a route-specific \texttt{zscore\_p99} value.}
  ...
  Isolation Forest was selected over alternative unsupervised detectors ---
  Local Outlier Factor (LOF), One-Class SVM (OCSVM), and autoencoder-based
  methods --- on four grounds specific to this deployment context.
  First, its training and inference complexity is linear in the number of
  samples ($\mathcal{O}(n \cdot t)$ for $t$ trees), making retraining
  feasible within the 6-hour Prefect schedule on homelab hardware without
  dedicated GPU.
  Second, isolation-based methods are robust to irrelevant features and
  do not require strong distributional assumptions (e.g.\ Gaussianity),
  unlike OCSVM whose decision boundary is sensitive to kernel choice and
  feature scaling \cite{isolation_survey_2025}.
  Third, the \texttt{contamination} hyperparameter provides a principled
  lever to encode the prior anomaly rate, which UrbanPulse estimates
  independently from the Z-score baseline ($\approx 2\%$); this prior
  was confirmed by Optuna TPE optimisation and set to $0.01$.
  Fourth, a recent survey of isolation-based anomaly detection methods
  \cite{isolation_survey_2025} confirms that Isolation Forest consistently
  achieves high scalability and noise robustness across multivariate
  time-series benchmarks, properties that directly match the Gold-layer
  feature space used here ($d = 7$, $n \approx 1{,}600$ records per route).
  ...
  \begin{algorithm}[H]
      ...
      \begin{algorithmic}[1]
          ...
          with contamination $= 0.01$, $T = 450$ trees,
          \texttt{max\_samples} $= 0.76$, \texttt{max\_features} $= 0.73$.
  ...
  The four IsolationForest hyperparameters
  (\texttt{contamination}, \texttt{n\_estimators},
  \texttt{max\_samples}, \texttt{max\_features})
  are selected via \textbf{Optuna} Bayesian optimisation
  \cite{optuna_paper} using the Tree-structured Parzen Estimator
  (TPE) sampler. For each of the 20 routes, 50 independent trials
  are evaluated; the per-route best parameters are aggregated by
  median to obtain a single global configuration.

  The objective function maximises the \emph{anomaly score gap}:
  \begin{equation}
    \mathcal{G}(\theta) =
      \overline{s}_{\text{normal}}(\theta) -
      \overline{s}_{\text{anomaly}}(\theta),
    \label{eq:score_gap}
  \end{equation}
  where $\overline{s}_{\text{normal}}$ and
  $\overline{s}_{\text{anomaly}}$ are the mean
  \texttt{decision\_function} outputs for the normal and anomalous
  subsets respectively, as determined by the candidate
  \texttt{contamination} threshold. Because both subsets are defined
  entirely within the ratio feature space
  $\mathcal{X} = (\rho_h, \rho_m, n_s, \sin/\cos(\text{hour}),
  \sin/\cos(\text{dow}))$, the objective is fully self-contained and
  ...
  offset $\theta^*$ such that exactly
  $\lfloor \texttt{contamination} \times N \rfloor$ training samples
  satisfy $s(\mathbf{x}) < 0$; a lower value therefore raises the
  effective decision threshold, flagging only the most extreme
  outliers and biasing the detector towards precision over recall.
  This trade-off is deliberate: the dual-signal fusion
  (Section~\ref{subsec:fusion}) already handles recall via the
  Z-score path, so the IForest component is tuned for
  high-confidence, low-false-positive output.
  ...
  Peri-urban}), the \texttt{iforest\_\allowbreak anomaly\_\allowbreak rate} of $0.01$ confirms
  that the contamination prior is honoured exactly by the decision threshold.
  The \texttt{model\_agreement} of $0.96$ indicates that the Z-score and
  IForest signals agree on 96\% of observations, with divergence concentrated
  ...
  \begin{figure}[htbp]
      ...
      Key indicators: \texttt{iforest\_\allowbreak anomaly\_\allowbreak rate}~$= 0.01$
      (matches contamination prior), \texttt{model\_agreement}~$= 0.96$,
      and \texttt{total\_samples}~$= 1{,}517$.}
